...
import { editOrReply } from '../utils/helper';
...
export const sendMainMenu = async (ctx: Context) => {
    ...
    const visibleItems = MENU_ITEMS.filter(item => {
        ...
        if (!item.permission) return true; // Public/Common feature
        return permissions.includes(item.permission);
    });

    // Always add Profile and Logout
    const bottomRow = [
        Markup.button.callback('👤 Hồ sơ (Me)', 'btn_me'),
        Markup.button.callback('👋 Đăng xuất', 'logout_action')
    ];

    // Build Grid
    const buttons = visibleItems.map(item => Markup.button.callback(item.label, item.callback));
    ...
    await editOrReply(ctx, msg, Markup.inlineKeyboard(grid));
};
